package br.com.senaisp.bauru.anderson.test;
...
import br.com.senaisp.bauru.anderson.classes.Produto;
...
public class CadastramentoProduto {
	...
	private static void excluirProduto(Scanner sc) {
		do {
	     	...
	     	if (prod !=null) {
	     		mostrarProduto(prod);
	     	    String conf = confirmarOperacao("Deseja exluir o registro (S/N)", sc);
	     		if (conf.toLowerCase().equals("s")) {
	     	     prod.apagarRegistro();
	     	}
	     	//Falta continua s/n
	...
	private static void consultarProduto(Scanner sc) {
		do {
			...
			String conf = confirmarOperacao("Dseja pesquisar novo produto? (S/N)", sc);
			if (conf.toLowerCase().equals("n")) {
				break;
			}
	...
	private static void cadastrarProduto(Scanner sc) {
		...
		do {
			try {
				...
				conf = confirmarOperacao("Deseja continuar?",sc);
				if (conf.toLowerCase().equals("n")) {
					break;
				}
			} catch (InputMismatchException e) {
